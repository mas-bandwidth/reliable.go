...
reliable.go implements packet acknowledgement, fragmentation and reassembly over UDP. It
...
In scope — bugs in this package, above all on the RECEIVE path: a packet that causes an
out-of-range index or slice, a panic, unbounded memory growth through fragment reassembly,
or a fragment accepted outside its declared bounds.

Go bounds-checks slices, so the interesting class here is a panic (a denial of service) or a
hostile packet that is accepted when it should be rejected.
